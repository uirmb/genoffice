...
- first persistence of an **unbound** new document: normal save with `newDocument: true`;
- a blank document created from `office:new` with `payload.file` is already bound to the Host-preallocated zero-byte node, so normal save uses that file identity and must not set `newDocument: true`;
- `mode: 'saveAs'` always means Save As regardless of whether the document is currently bound.
...
unbound new first save -> uc.fs.saveFileAs
prebound zero-byte new file -> uc.fs.saveCurrentFile
```
