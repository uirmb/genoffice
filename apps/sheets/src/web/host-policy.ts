...
function applyCapabilityClasses(capabilities: OfficeHostCapabilities): void {
  ...
  root.classList.toggle('office-can-export-xlsx', capabilities.exportXlsx)
  root.classList.toggle('office-can-close', capabilities.close)
}

export interface SheetsWebHostPolicyController {
  getCapabilities(): OfficeHostCapabilities
  destroy(): void
}

export function installSheetsWebHostPolicy(
  mode: WebRuntimeMode,
  bridge?: EditorIframeBridge,
): SheetsWebHostPolicyController {
  ...
  const apply = (patch?: Partial<OfficeHostCapabilities>): void => {
